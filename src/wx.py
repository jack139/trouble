...
class PostMsg:
    ...
    def text_process(self): # 处理文本消息回复
        ...
        if content_text==u'测试321':
            print 'here'
            #return self.reply_media([(u'公众号测试', u'', u'', u'http://xah-wx3.xmu.edu.cn:9090/hospitalPortal-web/xiamen/xianganyiyuan/index')])
            return self.reply_media([(u'公众号测试', u'', u'', u"https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxf28f23539c5821be&redirect_uri=http://mst.ylzinfo.cn/coreController/oauth2.htm&response_type=code&scope=snsapi_base&state=%7B'accountid':'gh_f1e3fe247105','redicturl':'http://xah-wx3.xmu.edu.cn:9090/hospitalPortal-web/xiamen/xianganyiyuan/index'%7D#wechat_redirect")])
    ...
    def event_process(self): # 处理事件请求
        ...
        if event=='CLICK':
            ...
            if self.key=='CLICK_WAIT':
                ...
                return self.reply_text(u"谢谢您对我们的支持。在下方聊天框中输入“我有建议+您的建议+姓名+联系电话”即可。")
            elif self.key=='CLICK_PDF':
                #content = [(u'标题2', u'', u'', u'http://wx.f8geek.com/live2')]
                #return self.reply_media([(u'下载会议通知文件', u'', u'', u'http://xah-wx.xmu.edu.cn/notice20180405.pdf')])
                return self.reply_media([(u'下载会议通知文件', u'', u'', u'https://pan.baidu.com/s/1baRw0Ftx780eHs1R0xUz8g')])
...
#------------------------------------------------
# 报名入口
class InitSignup: # signup.html
    def GET(self):
        param = get_param(web.input())
        raise web.redirect(get_redirect_loc('http://%s/wx/signup_init?%s' % (setting.wx_host, param)))
...
class Signup: 
    ...
    def POST(self):
        ...
        if user_data.get('session_id','')=='':
            ...
            if session_id==None:
                raise web.seeother('/wx/init_signup?%s' % param)
        else:
            session_id = user_data['session_id']

        url = 'http://xah-wx.xmu.edu.cn'+web.ctx.environ['REQUEST_URI']
        signature = genSignature(url)

        render = create_render(plain=True)
        return render.signup(session_id, param, signature)
