...
class handler:      
    def GET(self):
        ...
        if search_name!='':
            conditions['xm'] = { '$regex' : u'%s.*'%(search_name.replace('*','\\*').replace('?','\\?')), '$options' : 'i' }


        # 分页获取数据
        db_sku = db.job_sheet.find(conditions,
            sort=[('_id', -1)],
            limit=PAGE_SIZE,
        ...
        return render.job_list(helper.get_session_uname(), helper.get_privilege_name(), 
